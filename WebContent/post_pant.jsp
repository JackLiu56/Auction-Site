...
<%
   String description = request.getParameter("description");    
    ...
    int	waist= Integer.parseInt(request.getParameter("waist"));
       ...
       i = st.executeUpdate("insert into pant(product_id,brand,sizes,color,waist) values ((LAST_INSERT_ID()),'" + brand + "','" + sizes + "','" + color + "','" + waist + "');");
    if (i > 0) {}
     else {
        out.print("post failed!");
        response.sendRedirect("sellLanging.jsp");
    ...
    i=st.executeUpdate("insert into auction(start_date) values ('" + start_date +"');");
    if (i > 0) {
